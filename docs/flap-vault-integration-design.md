...
| `processRevenue()` | **OPERATOR_ROLE**（creator+Guardian+可授权） | `require(pendingBnb >= minProcessAmount)`；BNB→`Portal.swapExactInput{value}`（BUY 0→taxToken，**minOut = 同块 `quoteExactInput` × (1-maxSlippageBps)**）；若池不存在则 `deployPool(marketId, taxToken)`；`BasePool.deposit(...,self,self)` | reentrancy guard；operator 控时机/私有 mempool 防夹（quote 底线仅防价格剧变）；market 不存在 / swap / deposit 失败 → BNB 安全滞留 pendingBnb |
...
| **买回腿无外部可信价源**（tax token 无 Chainlink，Portal quote 是同块 spot） | **高** | `processRevenue` 改 **OPERATOR_ROLE**（控时机/私有 mempool）；minOut = 同块 quote × (1-slippage) 仅作单次损失上限兜底 |
...
8. 触发权限（**v3 变更**）：`processRevenue` 改 **OPERATOR_ROLE**（creator+Guardian+可授权；tax token 无外部价源，同块 quote 防不了三明治，对齐 Flap 官方对 buyback 类操作的建议）；`harvest` 保持任意人（feed 防护健全）。
