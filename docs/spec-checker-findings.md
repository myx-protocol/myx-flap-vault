...
| 002 | Commission fee recommendation | **PASS (zero-commission)** | The vault skims **no** commission from tax revenue — `receive()` accumulates 100% of `msg.value`; `process()` converts all of it to LP for holders. Zero ≤ the 6%-of-`msg.value` recommended cap; no justification needed. |
...
| 005 | `receive()` ≤ 1,000,000 gas, no external calls | **PASS** | `receive()` (`:125-128`) is `pendingBnb += msg.value; emit RevenueReceived(...)` — single SSTORE + event, no loop, no external call, no delegatecall, cannot revert on a non-zero value. Measured < 100k gas (`test_receive_gasUnder1M`). |
...
| 008 | Trigger service (`IFlapTriggerService`) | **N/A (changed from v4)** | v6 **removed** the trigger integration that v4 had. No `IFlapTriggerService` / `ITriggerReceiver` import, no `trigger(uint256)` callback, no `pendingTriggerId`, no scheduling. `process`/`feedDividend` are plain permissionless functions. Rule 008's callback-auth / replay / 2M-gas checks do not apply. |
