...
| 010 | V3 balance-delta accounting | **PASS** | `_sync()` recognizes revenue only as `currentBalance - pendingQuote` (`_quoteBalance() - pendingQuote`), never from `msg.value` or the raw balance directly. A wake (`receive()`/`sync()`) with zero delta is a silent no-op — `_sync()` returns early when `bal <= pendingQuote`. Every outflow decrements `pendingQuote` in the same transaction: `process()`, `_refillGas()`, `emergencySweepNative()`, `emergencyRescueToken()` (quote-token branch resets the baseline), and the native `scheduleProcess()` trigger fee. `vaultQuoteToken()` is immutable after init (Rule 001 row above). Tests: `test_bareTransfer_notRecognized_pingRecognizes`, `test_zeroDeltaPing_silentNoop`, `test_emergencyRescueToken_quote_resetsBaseline` (`test/MyxVaultErc20Quote.t.sol`). |
