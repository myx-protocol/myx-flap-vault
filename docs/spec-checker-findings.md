...
**Spec rules:** Flap VaultPortal rules 001–010 (`.agents/skills/flap-vault-spec-checker/references/rules/`).
Rule 010 (V3 ERC20-quote balance-delta accounting) is now in scope: the vault is a Flap spec V3
vault (`VaultBaseV3`, `vaultQuoteToken()`), so it must implement balance-delta recognition rather
than crediting `msg.value`/raw balance directly.
**Date:** 2026-06-14
...
|---|------|---------|----------|
| 001 | Vault inherits `VaultBaseV3`; `vaultUISchema()` + `description()` present; `vaultQuoteToken()` immutable after init | **PASS** | `MyxVault.sol` inherits `VaultBaseV3` and implements `vaultQuoteToken()` returning the `quoteToken` state var, which is set exactly once in `initialize` from the `quoteToken` argument the VaultPortal passes to `newVault` (NOT from `vaultData`'s `marketQuoteToken`, which only identifies the myx market) and has no setter — stable for the life of the vault per Rule 010 item 5. `vaultUISchema()` / `description()` remain present and v3-accurate. |
| 001 | Guardian granted every privileged role | **PASS** | `initialize` grants the guardian `DEFAULT_ADMIN_ROLE` + `EMERGENCY_ROLE`. **2026-09-15 (audit round 1, Finding 3):** the creator no longer holds any role — every emergency path can move holder-bound revenue, so it is Guardian-only. Guardian can call every gated function (`emergencyWithdraw`, `emergencySweepNative`, `emergencyRescueToken`, `setForward`). |
...
| 001 | No DOS via dev parameter manipulation | **PASS** | `maxSlippageBps` comes from the factory `GlobalConfig` (constructor-bounded to <= 10 000 bps). `minProcessAmount`, `gasThreshold` and `gasRefillAmount` are creator-supplied in `vaultData` at launch and immutable afterwards (**no post-init setter exists**), so the factory bounds them in `newVault`: `minProcessAmount != 0`, and for ERC20 quotes `gasRefillAmount <= config.maxGasRefillAmount`; the vault's `initialize` additionally requires `gasRefillAmount > gasThreshold` (ERC20) or both zero (native). The residual creator knob is that a large-but-capped `gasRefillAmount` diverts more of each batch (never more than `MAX_REFILL_SHARE_BPS` = 20%) into a BNB gas pool that only `EMERGENCY_ROLE` can sweep. **2026-09-15:** the creator holds no role, so that BNB is unreachable to them — only the Guardian can sweep it — and the refill is capped at 20% of the processed batch (not the backlog; audit round 1, Finding 9). `maxSlippageBps` is now hard-capped at `MAX_SLIPPAGE_BPS = 1_000` (10%) in both the factory constructor and `initialize` (Finding 4). No parameter combination can brick the vault: a refill that cannot be priced or that reverts is skipped (`GasRefillSkipped`) and the buyback still runs. |
...
| 003 | Fairness / sandwich-risk | **PASS (accepted bounded-sandwich)** | **2026-09-13:** `process()` is now trigger-only (`require(msg.sender == address(this))`, reached only from the FlapTriggerService callback in `trigger()`); the manual entry is `requestProcess()`, which schedules the callback and never swaps in the caller's transaction — a public caller cannot front-run + sandwich the vault's own buy (Flap review feedback). Tests: `test_process_directCall_reverts`, `test_requestProcess_native_schedulesAndPaysFromRevenue`, `test_requestProcess_erc20_fundsFeeWithValue`. Historic (pre-2026-09-13): `process()` was permissionless (`:143`). Buyback `minOut` is a same-block Portal quote × `(1 − maxSlippageBps)` (`_buyTaxToken`, `:262-285`) — bounds per-call deviation, cannot stop a same-block sandwich (honestly documented in-code, `:135-141`). No mutable privileged knob exists to pre-condition a sandwich (slippage/min frozen at init). The REWARD's fairness is the Flap Dividend `setShare` transfer hooks, not gameable on-chain. Accepted (no external feed exists for the tax token). **Per-call exposure bound (pre-audit 2026-09-12):** `process()` buys back at most `maxProcessAmount` per call and schedules its own follow-up for the remainder (`ProcessBatched`), so accumulated revenue (e.g. after a failed first callback) drains in bounded batches instead of one oversized swap. Tests: `test_process_capsAtMaxAndSchedulesFollowUp`, `test_process_capsAtMax_followUpPaidFromGasPool`, `test_process_noFollowUpWhenRemainderBelowMin`. |
| 004 | Literal `require` strings (no custom errors) | **PASS (fixed, Flap audit F1+F2)** | All custom errors removed; every user-facing `revert`/`require` now uses a bilingual literal string (`unicode"English / 繁體中文"`), so the Flap UI can decode and display revert reasons. Covers MyxVault (11 sites) + MyxVaultFactory (8 sites, incl. the previously-missed `OnlyVaultPortal` use-site). Condition inversions (`if(BAD) revert` → `require(GOOD)`) verified against the 95-test suite. |
...
| 008 | Trigger service (`IFlapTriggerService`) | **PASS (re-added)** | The vault implements `ITriggerReceiver`. `receive()` schedules `requestTrigger(now + 60s)`; the `trigger(uint256)` callback (a) **authenticates** `msg.sender == _getTriggerService()` (reverts `OnlyTriggerService`), (b) is **replay-safe** — clears `pendingTriggerId`/`hasPendingTrigger` before running and ignores stale/unknown ids, (c) runs `process()` under try/catch **within `getMaxCallbackGas()`**. **2026-09-14:** myx `deployPool` alone measures ~2.06M gas on mainnet, above the 2,000,000 callback cap, so pool deployment is kept out of callbacks by a one-way switch: no trigger is requested until `poolReady` is latched (`scheduleProcess` requires it; `requestProcess` reverts `Pool not deployed`), the pool is probed only while unlatched, and `ensurePoolDeployed()` (called by the MYX service once tax has accrued) deploys, latches and schedules the backlog. Steady-state callback ≈ 0.9M gas. Tests: `test/MyxVaultAutoTrigger.t.sol` incl. `test_trigger_wrongSender_reverts`, `test_trigger_staleId_ignored`, `test_trigger_gasWithinCallbackLimit`, and the `MyxVaultPoolGateTest` / `MyxVaultErc20PoolGateTest` suites. |
| 009 | Emergency controls (proxy-exempt branch) | **PASS** | Vault is a **BeaconProxy** (factory deploys `new BeaconProxy(beacon, ...)`, `MyxVaultFactory.sol:62-80`), so the Rule 009 mandatory `emergencyWithdrawNative`/`emergencyWithdrawToken` signatures are **exempt** — the upgrade path is the emergency mechanism. Upgrade authority is **Guardian-only**: the beacon is owned by the factory, `upgradeVaultImplementation` (`:147`) and `lockVaultUpgrades` (`:153`) are `onlyGuardian` (`_getGuardian()`), factory is non-upgradeable, no proxy admin / upgrader role / non-guardian owner remains. The vault additionally ships discretionary Guardian-only `EMERGENCY_ROLE` escapes (`emergencyWithdraw`, `emergencySweepNative`, and `emergencyRescueToken`) and, since 2026-09-15 (audit round 1, Finding 5), the rescue-mechanism forward switch: `setForward(address)` (DEFAULT_ADMIN_ROLE = Guardian) makes `receive()` redirect all incoming BNB to that address through a non-reverting low-level call and return before any accounting, so upstream tax dispatch keeps working during an incident. Tests: `MyxVaultForwardSwitchTest`. |
| 010 | V3 balance-delta accounting | **PASS** | `_sync()` recognizes revenue only as `currentBalance - pendingQuote` (`_quoteBalance() - pendingQuote`), never from `msg.value` or the raw balance directly. A wake (`receive()`/`sync()`) with zero delta is a silent no-op — `_sync()` returns early when `bal <= pendingQuote`. Every outflow decrements `pendingQuote` in the same function as the outflow, so a reverting outflow reverts its own decrement: `process()`, `executeGasRefill()` (the self-called helper holding the whole refill outflow — decrement, approve, swap, unwrap — so `_refillGas`'s try/catch commits all of it or none of it), `emergencySweepNative()`, `emergencyRescueToken()` (quote-token branch resets the baseline), and the native `scheduleProcess()` trigger fee. `vaultQuoteToken()` is immutable after init (Rule 001 row above). Tests: `test_bareTransfer_notRecognized_pingRecognizes`, `test_zeroDeltaPing_silentNoop`, `test_emergencyRescueToken_quote_resetsBaseline` (`test/MyxVaultErc20Quote.t.sol`). |
...
1. **`process()` permissionless — griefing/MEV (resolved 2026-09-13: trigger-only, see Rule 003).** Beyond the accepted bounded-sandwich (Rule 003),
